...
async def fetch_youtube_transcript(url: str) -> Optional[str]:
    """Return the transcript text for a YouTube video if available.

    The function prefers manually created English transcripts and falls back to
    autogenerated transcripts. If the installed ``youtube-transcript-api``
    version lacks ``list_transcripts``, it uses ``get_transcript`` instead.
    """

    try:
        video_id_match = re.search(
            r"(?:v=|\/|embed\/|shorts\/|youtu\.be\/|googleusercontent\.com\/youtube\.com\/(?:[0-8]\/)?)([0-9A-Za-z_-]{11})",
            url,
        )
        ...
        logger.info(
            f"Fetching YouTube transcript for video ID: {video_id} (from URL: {url})"
        )
        ...
        if hasattr(YouTubeTranscriptApi, "list_transcripts"):
            ...
            try:
                ...
                try:
                    ...
                    available_langs = [
                        t.language for t in transcript_list._transcripts.values()
                    ]
                    if available_langs:
                        logger.warning(
                            f"No English transcript for {video_id}. Available: {available_langs}. "
                            f"Trying first available: {available_langs[0]}."
                        )
                        try:
                            transcript_obj = transcript_list.find_generated_transcript(
                                [available_langs[0]]
                            ...
                            logger.warning(
                                "Could not fetch even the first available language transcript "
                                f"for {video_id}."
                            )
                    else:
                        logger.warning(
                            f"No English or any other language transcripts found for {video_id}."
                        )

            if transcript_obj:
                fetched_data = transcript_obj.fetch()
                # The object returned by fetch() is a FetchedTranscript, which is iterable
                # and yields FetchedTranscriptSnippet objects. Each snippet has .text.
                full_text = " ".join([entry.text for entry in fetched_data])

                if len(full_text) > config.MAX_SCRAPED_TEXT_LENGTH_FOR_PROMPT:
                    logger.info(
                        f"YouTube transcript for {url} (ID: {video_id}) truncated from {len(full_text)} "
                        f"to {config.MAX_SCRAPED_TEXT_LENGTH_FOR_PROMPT} chars."
                    )
                    full_text = (
                        full_text[: config.MAX_SCRAPED_TEXT_LENGTH_FOR_PROMPT] + "..."
                    )

                return (
                    f"(Language: {transcript_obj.language}) {full_text}"
                    if transcript_obj.language != "en"
                    and not transcript_obj.language.startswith("en-")
                    else full_text
                )

            logger.warning(
                f"No transcript could be fetched for YouTube video: {url} (ID: {video_id})"
            )
            return None
        ...
        video_id_for_log = (
            ...
            if "video_id_match" in locals() and video_id_match
            else "unknown_id_xml_err"
        )
        logger.error(
            f"Failed to parse YouTube transcript XML for {url} (ID: {video_id_for_log}): {e_xml}",
            exc_info=True,
        )
        return None
    except Exception as e:
        video_id_for_log = (
            video_id_match.group(1)
            if "video_id_match" in locals() and video_id_match
            else "unknown_id_gen_err"
        )
        logger.error(
            f"Failed to fetch YouTube transcript for {url} (ID: {video_id_for_log}): {e}",
            exc_info=True,
        )
        return None
