...
async def process_rss_feed(
    ...
    progress_msg : discord.Message
        Message to update with progress information.
    ...
    await progress_msg.edit(content=f"Fetching RSS feed: {feed_url}...", embed=None)
...
def setup_commands(bot: commands.Bot, llm_client_in: Any, bot_state_in: BotState):
    ...
    async def rss_slash_command(
        ...
        try:
            await process_rss_feed(interaction, final_feed_url, limit, progress_message)
        except Exception as e:
    ...
    async def allrss_slash_command(
        ...
        if queue_notice:
            ...
            await scrape_lock.acquire()
            acquired_lock = True
            progress_message = await interaction.followup.send(content="Starting RSS feed processing...")
        else:
            await scrape_lock.acquire()
            acquired_lock = True
            await interaction.response.defer(ephemeral=False)
            progress_message = await interaction.edit_original_response(content="Starting RSS feed processing...")

        try:
            for name, feed_url in DEFAULT_RSS_FEEDS:
                await progress_message.edit(content=f"Processing feed: {name}...", embed=None)
                while True:
                    processed = await process_rss_feed(interaction, feed_url, limit, progress_message)
                    if not processed:
                        break
        except Exception as e:
            logger.error(f"Error in allrss_slash_command: {e}", exc_info=True)
            await progress_message.edit(content=f"Failed to process RSS feeds. Error: {str(e)[:500]}", embed=None)
        finally:
            if acquired_lock:
                scrape_lock.release()
