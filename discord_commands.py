...
def setup_commands(bot: commands.Bot, llm_client_in: Any, bot_state_in: BotState):
    ...
    @bot_instance.tree.command(name="podcastthatshit", description="Triggers the podcast that shit instruction.")
    async def podcastthatshit_slash_command(interaction: discord.Interaction):
        await interaction.response.send_message("Triggering...", ephemeral=True)
        await interaction.channel.send("Podcast that shit")
