warning: this project changes with my mood. use at your own risk. also, f*ck openai for what they did to plus users access. i'd recommend only using this as a personal bot. dick around with models and token limits etc. let your freak flag fly. in any case, be aware that oai does offer free api tokens for things that are allowed to be trained on... like... a lot of tokens... especially for smaller models. but also be aware that discord disallows outputs to be trained in ai. an app that gives other people's conversation data to ai training is not permitted. don't do it. keep your bot private if you are utilizing free tokens. that or just use local. lm studio is real cool. anyway. i'm not in a good mood and have cancelled my oai plus sub. i'm angry at the removal of tools i used to have. tools i learned nuance of. now it's a monolithic garbage text generator. i hate gpt-5. also, it breaks the tts on many outputs. dunno why. maybe they use a dumb token in gpt-5. mini seems to work fine. but oai is suicidal recently, so... i'd lean toward local if i were you. k. have fun. bai. i'm off to try and make a living somewhere else now. free software doesn't pay. i'm a pain patient and hate life right now. 
update: they gave plus users back model access except 4.5! this is acceptable to me. that thing was research preview anyway. 

Sponsor me not dying here:
