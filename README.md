...
7.  **RAG and ChromaDB Management (`rag_chroma_manager.py`)**:
    ...
    *   `retrieve_and_prepare_rag_context()`: Given a query, searches relevant collections for pertinent information and synthesizes it into a context string for the LLM. The synthesis step now receives the current date so it can express how long ago memories occurred. It also detects relative date phrases (e.g. "yesterday", "last week") and explicit dates (e.g. "March 3, 2024") and prioritizes tweets, news, RSS summaries, chat history, and timeline entries from that time range before falling back to standard retrieval.
